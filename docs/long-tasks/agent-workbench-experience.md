...
- Current checkpoint: 第一批已随 v1.74.0 发布；用量/context 上移已随 v1.75.1 发布；输入控件对齐和会话侧栏归属修正已由 PR #274 合并并通过全部 CI/远程 GUI，等待立即切版与公开产物验收。
- Next owner: Release Ops 发布并验收 PR #274 所在批次；第二批继续 R17 单一 pane arbiter 与 R18 共用任务活动组件。
...
- [ ] 发布并验证包含 PR #274 的公开安装产物；在产物可下载且版本元数据一致前，该修正仍为 `not live`。
...
- Release evidence:
  ...
  - PR #274 已合并但尚未进入公开产物，当前状态为 `not live`；不得用 main 或绿色 CI 代替发布验收。
...
- validation result: 第一批与用量位置反馈已公开发布；PR #274 的全量测试、build、治理、本地真实 App 与远程 GUI 通过，公开产物仍待验收；R17/R18 和补充视口证据仍待后续批次。
