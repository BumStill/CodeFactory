...
## 系统边界
- CodeFactory 是 Windows 优先的本地 AI 编程 Agent 桌面客户端。
- 桌面壳使用 Tauri 2，前端使用 React + TypeScript + Vite，后端使用 Rust。
- 模型接入首期只走 OpenRouter 的 OpenAI 兼容 Chat Completions 和 SSE。
- 会话、消息、工具调用和成本统计使用 SQLite 本地存储。
- API Key 使用 Windows Credential Manager，不落明文文件。
...
- OpenRouter provider/model 差异由模型适配层处理，UI 不直接依赖某一家模型的私有字段。
- Windows 10/11、WebView2 版本和安装包签名状态必须纳入 release evidence。
