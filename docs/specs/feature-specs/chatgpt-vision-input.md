...
| --- | --- | --- | --- | --- |
| CF-VISION-R1 | 解决图片根本没有识别到 | ChatGPT Responses 用户消息必须把内部 `image_url` 转换为 `input_image` | Rust ChatGPT transport | 失败优先单元测试断言字段和顺序 |
...
- Field-level assertions: `type=input_image`、`image_url=data:image/...;base64,...`、混合 part 顺序不变
- Failure assertion: 空图片 URL 不进入 `input_image`；缺失、非图片或超限文件继续由既有附件层降级为文本

## Primary User Path

用户在 CodeFactory 中附加本地图片并输入问题，选择 ChatGPT 订阅模型发送；模型请求必须同时收到问题文本和图片像素数据，并能基于图片内容回答。
