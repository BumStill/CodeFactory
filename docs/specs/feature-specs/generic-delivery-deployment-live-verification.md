...
本规格中的 `blocked` 仅保留为旧 tool/delivery projection 兼容值。未达到 requested ceiling
时 objective 必须按 `objective-recovery-control-plane.md` 保持 `waiting_system`、
`core_input_required` 或 `needs_business_decision`；技术状态不得成为人工恢复终点。
...
- **REQ-DEL-006 Fail closed**：未配置部署观察或 live verifier、部署仍 pending、线上断言不匹配时，不得返回 `delivered` 或声称上线；局部工具可返回兼容 `blocked/waiting`，但 objective 必须保留已完成步骤并进入 system-owned remediation。
...
2. `deliver_changes` 先无副作用 preflight；remote/review 通道缺失或配置不可读时 fail closed 并进入 typed remediation。更高层执行器缺失时保留已达证据与 requested ceiling；结果同时显示 requested/effective/reached，未达到 requested 时 objective 保持 system-owned 或必要输入等待。
...
- delivery 局部 blocked/waiting → 结构化 metadata 同时出现在工具结果、stream event、前端 tool-call state、`tool_calls.metadata` 与 objective decision；技术状态无人工恢复 CTA。
