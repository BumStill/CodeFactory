...
| CF-DR-R14 | 程序内更新先进入持久队列；只有 backend 持有精确 target mutation permit，且本地执行 owner、未过期 Objective lease、**mutation-capable** Delivery lease、权限、浏览器与终端均归零后，才可下载、安装和重启。Delivery takeover 在 `claim_epoch > reconciled_claim_epoch` 时只拥有 observe-only 租约，不能执行本地或远端 mutation，必须允许跨重启且不得伪装成执行中 blocker；`claim_epoch = reconciled_claim_epoch` 的未终态 live lease 继续 fail closed。无 live lease 的 `waiting_core_input`、`waiting_authorization`、`waiting_business_decision` 与 `legacy_orphan` 同样是可跨重启的 durable wait；安全状态未知时 fail closed | update safety command、updater store、状态 UI | frontend retry contract + Rust owner/lease aggregation + 历史 SQLite takeover fixture + real App |
