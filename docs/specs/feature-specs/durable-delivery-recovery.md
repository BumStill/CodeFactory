...
| CF-DR-R14 | 程序内更新先进入持久队列；只有 backend 持有精确 target mutation permit，且本地执行 owner、未过期 Objective lease、**mutation-capable** Delivery lease、权限、浏览器与终端均归零后，才可下载、安装和重启。Delivery takeover 在 `claim_epoch > reconciled_claim_epoch` 时只拥有 observe-only 租约，不能执行本地或远端 mutation，必须允许跨重启且不得伪装成执行中 blocker；只有 `claim_epoch > 0` 且 `claim_epoch = reconciled_claim_epoch` 的未终态 live lease 才继续 fail closed，历史零 epoch 行从未持有 mutation claim。无 live lease 的 `waiting_core_input`、`waiting_authorization`、`waiting_business_decision` 与 `legacy_orphan` 同样是可跨重启的 durable wait；安全状态未知时 fail closed | update safety command、updater store、状态 UI | frontend retry contract + Rust owner/lease aggregation + 历史 SQLite takeover fixture + real App |
...
| CF-DR-R21 | 本地 `stage/commit` 也是不可重放副作用：先用 `commit-tree` 生成不移动 ref 的精确 child SHA，再在语义 ref mutation 前写入绑定 objective/run/repo/worktree/previous head/change-set/staged tree/branch/commit-message digest/**expected child SHA** 的 write-ahead receipt，最后用 CAS `update-ref` 提交。进程若在 intent 落盘后、ref CAS 前退出，替代 owner 只有在 exact unresolved intent、当前 live lease/owner/claim epoch、未取消/未停泊 Objective 与 staged index tree 同时匹配时，才能在 SQLite writer fence 内完成预先确定的 ref CAS；旧 owner 或取消并发必须零 Git mutation。进程若在 ref 已移动、DeliveryRun head revision 落盘前退出，则只有当前 clean HEAD 与 receipt 的 expected child SHA 完全相等，且 parent/tree/message/repo/worktree 全部匹配，才可原子修订 identity 且恰好一次；同 parent/tree/message 但不同 author/time/SHA 仍是外来提交。若中断发生在 push 前且既有 canonical PR/remote 仍精确指向 receipted parent，替代 owner必须把 parent allowance 写入 durable identity revision，使第二次 lease loss 后仍能只读续接同一 PR；远端一旦推进到 child，parent allowance 即失效。无 receipt、非后代、dirty、tree/message/SHA 不符或外来提交一律零远端 mutation。同一 `delivery_identity_conflict` 只按不同 `claim_epoch` 消耗预算，同一 lease 内重复 callback 不得提前耗尽；达到上限时 Objective、DeliveryRun、turn/run-control 必须同事务稳定停泊，后续 startup poll 不得再增加 `claim_epoch`；只有可审计的新凭据或能力代际才能重新准入 | local commit intent、DeliveryRun identity revision、takeover planner、delivery recovery executable | failure-first Rust + 三进程 real Git/SQLite hard kill + exact release executable + installed artifact receipt |
