...
| CF-WB-R5 | 结果 footer 分开表达“计划步骤执行度”和“证据完整性”，并明确区分 objective completed、system-owned 未完成、必要输入/业务决定与 cancelled；未完成不得使用成功语义，技术状态不得显示用户恢复 CTA | TurnResultSnapshot | component + adversarial fixture |
...
| CF-WB-R7 | composer、queue、draft scope、运行策略与 context 收敛成一个 raised 操作表面；输入区 footer 承载模型/思考/权限与 context 圆环；曲别针、单行文本与发送按钮共享 32px 视觉基准；placeholder 不列文件格式 | Workspace + MessageInput | component geometry + real app |
...
| CF-WB-R20 | 模型、思考强度与会话权限位于 composer，作用域和生效时点可理解；运行中切换模型明确标示“下一回合生效”，权限升级仍显示完整风险说明 | composer runtime controls | component + real app |
...
| 场景 | 正常路径 | 边界路径 |
| --- | --- | --- |
| Result | CompletionArbiter 通过且 evidence 完整才显示已完成 | plan 6/6 但证据不足仍显示 system-owned 继续处理；只有 typed input/decision 显示用户动作 |
| Context | 58% 圆环为中性/progress，累计 Token 进入详情 | 75% warning、90% danger、数据未知显示可理解空态 |
| Composer | 正常发送、附件、模型/思考/权限、context 圆环 | streaming 下一回合生效、queue、stop、权限升级、错误 |
