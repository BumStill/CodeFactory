...
- GUI 验证启动后主动防止空闲休眠；PR 和发布流水线固定使用远端 macOS 可见会话，不把解锁用户电脑作为依赖。
...
- 不取消真实 App 的视觉和交互验收，而是把它迁移到不受本机锁屏影响的远端可见会话。
...
4. PR、发布和安装包复验由远端流水线继续推进，用户锁屏不会把确定性改进停在本地分支。
...
- GUI 行为变更必须由远端 macOS 可见会话在本次精确 App bundle 上生成窗口与截图证据；发布产物再执行一次同口径 smoke。
