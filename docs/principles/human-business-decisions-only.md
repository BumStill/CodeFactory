...
应用更新同样属于系统拥有的技术动作，不得用“新版本可用”覆盖正在执行的用户目标。更新包可以后台下载，但安装和重启前必须统一核对聊天回合、任务调度器、交付租约、权限等待、受管浏览器和终端；任一仍活跃时自动等待安全点，归零后续接安装，不要求用户再次点击。更新后的恢复能力只能作为异常兜底，不能替代更新前的无中断门禁。
