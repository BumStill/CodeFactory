  ...
  return (
    <div className="h-full flex flex-col bg-surface-0">
      <header className="flex items-center gap-3 px-4 py-2.5 border-b border-border bg-surface-1 shrink-0">
        ...
        <span className="text-body font-semibold text-gray-200">我的画像</span>
      </header>
          ...
          <section className="rounded-lg border border-accent/30 bg-accent/5 p-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="min-w-0">
                <h2 className="text-label font-semibold text-gray-400">进化审查</h2>
              ...
              <button
                ...
                className="rounded bg-accent px-3 py-1.5 text-label text-white hover:bg-accent-hover disabled:opacity-50"
              >
                前往进化审查
              </button>
            </div>
  ...
  return (
    <section>
      ...
      {scope === "project" && !selectedCwd ? (
        <p className="text-label text-gray-500 text-center py-6">选一个项目以查看项目偏好</p>
      ) : loading ? (
        <p className="text-label text-gray-500 text-center py-6">加载中…</p>
      ) : (
      ...
      {error && <p className="mt-2 text-label text-red-700 dark:text-red-300">{error}</p>}
    </section>
  );
}
  ...
  return (
    <div className="flex items-start gap-4 px-4 py-3">
      <div className="w-24 shrink-0">
        <div className="text-label font-medium text-gray-300">{label}</div>
        <span className={`mt-1 inline-block text-caption px-1.5 py-0.5 rounded ${badge.cls}`}>
          {badge.text}
        </span>
      </div>
      <div className="flex-1 min-w-0">
        {editing ? (
          <input
            ...
            className="w-full bg-surface-3 border border-border rounded px-2 py-1 text-body text-gray-200 outline-none focus:border-accent"
          />
        ) : (
          <button
            onClick={() => setEditing(true)}
            className="text-body text-gray-200 hover:text-accent text-left w-full"
          >
            {pref.value || <span className="text-gray-500 italic">（点击设置）</span>}
          </button>
        )}
        <div className="text-caption text-gray-500 mt-0.5">{hint}</div>
      </div>
    </div>
  );
}
      ...
      <div className="rounded-lg border border-border bg-surface-1 overflow-hidden">
        ...
        <div className="px-4 py-3 border-b border-border bg-surface-2 flex items-center gap-2">
          ...
          <select
            ...
            className="flex-1 bg-surface-3 border border-border rounded px-2 py-1 text-label text-gray-200 outline-none focus:border-accent font-mono"
          >
        ...
        <div className="p-4">
          {loading ? (
            <p className="text-label text-gray-500 text-center py-8">加载中…</p>
          ) : !selectedCwd ? (
            <p className="text-label text-gray-500 text-center py-8">
              选一个项目以查看记忆
            </p>
          ) : (
            <>
              <textarea
                ...
                className="w-full bg-surface-2 border border-border rounded px-3 py-2 text-body text-gray-200 font-mono leading-relaxed outline-none focus:border-accent resize-y"
              />
              <p className="mt-2 text-caption text-gray-500">
          ...
          {error && (
            <p className="mt-2 text-label text-red-700 dark:text-red-300">{error}</p>
          )}
        </div>
    ...
    <section>
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-label font-semibold text-gray-400">
          学习日志
          {pending.length > 0 && (
            <span className="ml-2 text-caption font-normal text-accent normal-case">
              {pending.length} 条待审
            </span>
          )}
        </h2>
      ...
      {!selectedCwd ? (
        ...
        <div className="rounded-lg border border-dashed border-border bg-surface-1 px-6 py-10 text-center">
          ...
          <p className="text-body text-gray-400 font-medium mb-1">
            {events.length === 0
              ? "暂无学习记录"
              : `没有「${filter === "memory" ? "记忆" : filter === "preference" ? "偏好" : "模式"}」类型的记录`}
          </p>
          <p className="text-label text-gray-500 max-w-md mx-auto leading-relaxed">
        ...
        <div className="space-y-3">
          ...
          {sessionGroups.length > 0 && (
            <details className="rounded-lg border border-border bg-surface-1">
              <summary className="px-4 py-2 text-label text-gray-500 cursor-pointer hover:text-gray-300 select-none">
              ...
              <div className="border-t border-border divide-y divide-border">
                {sessionGroups.map(([sid, list]) => (
                  <details key={sid} className="px-4 py-2 group" open>
                    <summary className="text-caption text-gray-500 cursor-pointer hover:text-gray-300 select-none flex items-center gap-2">
                    ...
                    <ul className="mt-1.5 space-y-1">
                      {list.map((e) => (
                        <li key={e.id} className="text-caption pl-3">
                          <span
                            className={
                              e.status === "accepted"
                          ...
                          <span className="text-caption text-gray-600 mr-1">
...
export function SelfImprovementSection() {
  ...
  return (
    <section>
      ...
      {error && (
        <p className="mb-2 text-label text-red-700 dark:text-red-300">{error}</p>
      )}

      {proposal === null ? (
        <div className="rounded-lg border border-dashed border-border bg-surface-1 px-6 py-10 text-center">
          <Lightbulb size={20} className="text-gray-600 mx-auto mb-3" />
          <p className="text-body text-gray-400 font-medium mb-1">还没有生成提案</p>
          <p className="text-label text-gray-500 max-w-md mx-auto leading-relaxed">
    ...
    <section>
      ...
      {proposals === null ? (
        <div className="rounded-lg border border-dashed border-border bg-surface-1 px-6 py-10 text-center">
          ...
          <p className="text-body text-gray-400 font-medium mb-1">还没有扫描</p>
          <p className="text-label text-gray-500 max-w-md mx-auto leading-relaxed">
  ...
  return (
    <div className="rounded-lg border border-accent/40 bg-accent/5 p-4">
      ...
      <div className="rounded bg-surface-2 border border-border px-3 py-2 mb-3">
        ...
        <p className="text-label text-gray-200 font-mono leading-relaxed">{event.suggestion}</p>
        {isPref && event.pref_key && (
          <p className="mt-1 text-caption text-gray-500 font-mono">
