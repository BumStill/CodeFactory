...
fn resolve_document_path(cwd: &Path, requested: &str) -> Result<(PathBuf, String), AppError> {
    ...
    let resolved = std::fs::canonicalize(&candidate)
        .map_err(|e| AppError::Other(format!("document unavailable: {e}")))?;
    if !resolved.starts_with(&root) {
        return Err(AppError::Other("document is outside the workspace root".into()));
    }
    if !resolved.is_file() {
        return Err(AppError::Other("document is not a file".into()));
    }
...
pub async fn read_document(cwd: String, path: String) -> Result<DocumentPreview, AppError> {
    ...
    let (resolved, relative_path) = resolve_document_path(root, &path)?;
...
mod tests {
    ...
    fn resolves_relative_document_inside_workspace() {
        ...
        std::fs::write(root.path().join("docs/plan.md"), "# Plan").unwrap();
        let (path, relative) = resolve_document_path(root.path(), "docs/plan.md").unwrap();
        assert!(path.ends_with("docs/plan.md"));
        assert_eq!(relative, "docs/plan.md");
    }
    ...
    fn rejects_documents_outside_workspace() {
        ...
        let error = resolve_document_path(root.path(), path.to_str().unwrap()).unwrap_err();
