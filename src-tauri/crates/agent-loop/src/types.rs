...
//! Wire types shared by the desktop agent loop and the headless sidecar.
//!
//! Moved here verbatim from `src/openrouter/types.rs` (keystone slice 4.1) so
//! the tauri-free shared loop crate owns them. `crate::openrouter::types`
//! re-exports this module, so every existing `crate::openrouter::types::*` path
//! keeps compiling. Behaviour and serde representation are unchanged.
